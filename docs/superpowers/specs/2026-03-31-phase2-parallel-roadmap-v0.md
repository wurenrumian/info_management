...
在不引入高复杂度依赖的前提下，完成 5 个低耦合业务模块并行开发，形成“学生可用 + 管理可用 + 可审计 + 可扩展”的业务中层。
...
并行原则：
- Phase 2-0（文档库）先行开发，为 A/B/C/D/E 提供文件基础设施
- 5 个业务模块都复用 Phase 1 权限/scope + Phase 2-0 文件服务，不互相等待业务实现
- 先统一接口风格、错误码、审计字段，再并行写代码

建议节奏：
1. 第 0 周：统一开发规范与目录约定（本文档 + development-standard）
2. 第 0.5 周：Phase 2-0 文档库开发（文件基础设施）
3. 第 1-2 周：五模块并行开发核心 API（各自最小闭环）
4. 第 3 周：合同测试与联调
5. 第 4 周：小程序/网站前端对接
...
1. 先确认本路线图与开发规范  
2. 各模块负责人独立补 v0 spec + implementation plan（A/B/C/D/E），命名与位置遵循开发规范
